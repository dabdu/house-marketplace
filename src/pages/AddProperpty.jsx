...
function AddProperty() {
  ...
  const onSubmit = async (e) => {
    ...
    // if (houseDoc.length === 0) {
    //   setLoading(false);
    //   toast.error("Upload at least  one house document");
    //   return;
    // }
    // let geolocation = {};
    // let location;
    ...
    //     `https://maps.google/maps/api/geocode/json?address=${address}&key=AIzaSyAILQW6vxWWbkXtjgoRuGbpBJrOBoBfX8s`
    //   );
    ...
    // store House Documents in firebase
    // const storeDoc = async (doc) => {
    //   return new Promise((resolve, reject) => {
    //     const storage = getStorage();
    //     const fileName = `${auth.currentUser.uid}-${doc.name}-${uuidv4()}`;
    //     const storageRef = ref(storage, "houseDocs/" + fileName);
    ...
    //   });
    // };
    // const DocUrl = await Promise.all(
    //   [...houseDoc].map((doc) => storeDoc(doc))
    // ).catch(() => {
    //   setLoading(false);
    ...
    toast.success("Property Added Succesflly");
  ...
  return (
    <div className="profile">
      <header>
        <p className="pageHeader">Add New Property</p>
      </header>
